...
describe('Dictionary', () => {
  ...
  beforeEach(() => {
    ...
    global.chrome.storage.local.remove.mockImplementation((keys, callback) => callback && callback());
  });
  ...
  describe('clearFullDictionary', () => {
    ...
    test('should remove from storage', async () => {
      ...
      expect(chrome.storage.local.remove).toHaveBeenCalledWith(['fullDictionary'], expect.any(Function));
    });
  });
